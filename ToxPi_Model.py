          ...
          with arcpy.da.SearchCursor(inFeatures, ['SHAPE@XY', "STATE_FIPS", "STATE_NAME"]) as scur:
            for i, row in enumerate(scur): #for each input data point
                ...
                for j, f in enumerate(inFields): #loop through slice fields
                    ...
                    median = round(medians[j], 2)                   #get USAmedian for slice
                    statemedian = round(statemedians[j][id_], 2)    #get statemedian for slice 
            ...
            with arcpy.da.SearchCursor(inFeatures, ['SHAPE@XY', "Name", "STATE_FIPS"] + allFlds) as scur:
              for i, row in enumerate(scur): #for each input data point
                ...
                for j, f in enumerate(inFields): #loop through slice fields
                    v = round(row[count+4], 2) #get toxpi score for slice
                    weight = str(inputWeights[count]*100/360) + "%" #get weight for slice as percentage
                    rank = ranks[j][id_] #get rank in the US for slice score
                    median = round(medians[j], 2) #get US median for slice
                    statemedian = round(statemedians[j][state_id], 2) #get state median for slice
                    count = count + 1
...
def adjustinput(infile, outfile):
    ...
    del df['Source']
    df.columns = [header.replace("&", "and").replace("\"", "").replace(":","").replace("-","").replace(";","").replace(" ","_") for header in df.columns]
    ...
    weights = []
    infields = []
    keywords = ["ToxPi_Score", "HClust_Group", "KMeans_Group", "Name", "Longitude","Latitude","FIPS", "Tract"]
...
def ToxPiCreation(inputdata, outpath):  # ToxPi_Model
    ...
    with arcpy.da.SearchCursor(tmpfileremapped, ["STATE_FIPS","CNTY_FIPS",uniqueid,] + list(category_names)) as scur:
      ...
      for z, cat in enumerate(category_names):
        ...
        countyUSAmedians[z] = round(statistics.median([v for k, v in toxpidata[z].items()]), 2)
        ...
        for i, state in enumerate(sectionedtoxpidata[z].keys()):
            if i == 0:
                ...
                countyStatemedians[z][state] = round(statistics.median([v for k, v in sectionedtoxpidata[z][state].items()]), 2)
