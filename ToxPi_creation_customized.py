...
def ToxPiCreation(inputdata, outpath):  # ToxPi_Model
    ...
    if uniqueidtype == "FIPS":
      ...
      fields = [i.name for i in arcpy.ListFields(boundaries)]

      arcpy.management.DeleteField(boundaries, 'OBJECTID')
      arcpy.management.DeleteField(boundaries, 'NAME')
      fields = fields[8:-2]

      for i in fields:
          arcpy.management.DeleteField(boundaries, i)
      arcpy.management.JoinField(tmpfileremapped, uniqueid, boundaries, "FIPS")


    elif uniqueidtype == "Tract":
      boundaries = arcpy.management.CopyFeatures("https://services1.arcgis.com/aT1T0pU1ZdpuDk1t/arcgis/rest/services/USA_Census_Tract_Boundaries/FeatureServer/0", geopath + "\censusypolylayer")
      fields = [i.name for i in arcpy.ListFields(boundaries)]
      ...
      ToxPiFeatures(inFeatures=tmpfileremapped, outFeatures=tmpfileToxPi, uniqueID = uniqueid, uniqueidtype = uniqueidtype, inFields=infieldsrevised, inputRadius=int(radius), radiusUnits="MILES", inputWeights=inweights, inFieldsrename = infields)
      ...
      y["layerDefinitions"][0]["popupInfo"] = json.loads(GetPopupInfo("County", position = "foreground", data = "minimal"))
    ...
    with arcpy.da.SearchCursor(tmpfileremapped, ["STATE_FIPS","CNTY_FIPS",uniqueid,] + list(infieldsrevised)) as scur:
      for j, row in enumerate(scur):
            for i in range(len(infieldsrevised)):
                if j == 0:
      ...
      countyUSAmedians = [None]*len(infieldsrevised)
      countyStatemedians = []
      countyrankslist = []
      stateranklist = []
      #statemediansmedian = []*len(infieldsrevised)

      #calculate stats for each slice type
      for z, cat in enumerate(infieldsrevised):
    ...
    y["layerDefinitions"][0]["minScale"] = "3000000"
    ...
    statebackground = arcpy.management.MakeFeatureLayer(stateboundaries,"State Boundaries")
    countytoxpiinfo = arcpy.analysis.Select(countytoxpilyr, "countytoxpiinfo", '"SliceName" = \'ToxPi Score\'')
    ...
    arcpy.management.CopyFeatures(statebackgroundpartial, geopath + "\statepolygons")
    ...
    ToxPiFeatures(inFeatures=geopath + "\statepointstmp", outFeatures=tmpfileToxPiLg, uniqueID="STATE_FIPS", uniqueidtype = uniqueidtype, inFields=infields, inputRadius=int(radius)*30, radiusUnits="MILES", inputWeights=inweights, inFieldsrename = infields, ranks = countyrankslist, medians = countyUSAmedians, statemedians = countyStatemedians, stateranks = stateranklist, Large = True)
    ...
    y = json.loads(data)
    #set that minimum extent for county toxpi ring feature layer
    y["layerDefinitions"][0]["maxScale"] = "3000000"
    y["layerDefinitions"][0]["renderer"]["symbol"]["symbol"]["symbolLayers"][0]["color"]["values"][0] = "0"
    y["layerDefinitions"][0]["renderer"]["symbol"]["symbol"]["symbolLayers"][0]["color"]["values"][1] = "0"
    y["layerDefinitions"][0]["renderer"]["symbol"]["symbol"]["symbolLayers"][0]["color"]["values"][2] = "0"
    y["layerDefinitions"][0]["renderer"]["symbol"]["symbol"]["symbolLayers"][0]["color"]["values"][3] = "100"   
    ...
    #stateinfo = arcpy.management.AddJoin(geopath + "\statepolygons", "STATE_FIPS", statetoxpilyr, "STATE_FIPS")
    fields = [i.name for i in arcpy.ListFields(geopath + "\statepolygons")]

    fields = fields[4:-2]
    for i in fields:
      arcpy.management.DeleteField(geopath + "\statepolygons", i)
    for i, name in enumerate(infieldsrevised):
      arcpy.management.AddField(geopath + "\statepolygons", name, "DOUBLE", field_alias = infields[i])
    fields = [i.name for i in arcpy.ListFields(geopath + "\statepolygons")]

    with arcpy.da.UpdateCursor(geopath + "\statepolygons", ["STATE_FIPS"] + infieldsrevised) as cursor:
      for row in cursor: #for each input data point     
        id_ = row[0]    #get state FIPS for point
        for j,f in enumerate(infieldsrevised): #loop through slice fields
          row[j+1] = round(countyStatemedians[j][id_], 3)    #get statemedian for slice            
        cursor.updateRow(row)
    for i, name in enumerate(infieldsrevised):
          arcpy.management.AlterField(geopath + "\statepolygons", name, infieldsrevised[i][:31], infields[i])
    statebackground=arcpy.management.MakeFeatureLayer(geopath + "\statepolygons","State Boundaries")
    ...
    y["layerDefinitions"][0]["popupInfo"] = json.loads(GetPopupInfo("State", fields = infields, infieldsrevised = infieldsrevised))
    ...
    y["layerDefinitions"][0]["popupInfo"] = json.loads(GetPopupInfo("State", position="foreground"))
    ...
    joinedtoxpiinfo = arcpy.management.JoinField(boundaries,"FIPS", tmpfileremapped, "FIPS", [])
    for i, name in enumerate(infieldsrevised):
          arcpy.management.AlterField(joinedtoxpiinfo, name, infieldsrevised[i][:31], infields[i])
    backgroundpartial = arcpy.analysis.Select(joinedtoxpiinfo, geopath + r"\boundaryselect", '"ToxPi_Score" IS NOT NULL')
    background = arcpy.management.MakeFeatureLayer(backgroundpartial,"Local Boundaries")
    ...
    y["layerDefinitions"][0]["popupInfo"] = json.loads(GetPopupInfo("County", position = "background", fields = infields, infieldsrevised = infieldsrevised))
    ...
    os.remove(outpathtmp + r"\boundarypolygons.lyrx")
    os.remove(outpathtmp + r"\statepolygons.lyrx")
    os.remove(outpathtmp + r"\staterings.lyrx")
    os.remove(outpathtmp + r"\StateToxPi.lyrx")
    os.remove(outpathtmp + r"\countyrings.lyrx")
    os.remove(outpathtmp + r"\midrings.lyrx")
    os.remove(outpathtmp + r"\MidSizedToxpi.lyrx")
    os.remove(outpathtmp + r"\ToxPiResultsAdjusted.csv")
